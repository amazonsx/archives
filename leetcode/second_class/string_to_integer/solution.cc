    ...
    public:
        ...
        int atoi(const char *str) {
            ...
            while (str[start] != '\0') {
                ...
                res = res*10 + subval;
                if (res > numeric_limits<int>::max())    break;
                start ++;
                count ++;
            }
            int ret = (signed)res;
            if ( res > numeric_limits<int>::max()) {
                if (flag == 1)  return numeric_limits<int>::max();
...
int Solution::atoiX(const char *str) {
    if (!str) return 0;
    int res = 0;
    char c = str[0];
    int flag = 1;
