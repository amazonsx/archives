...
class Solution {
    public:
        ...
        int maxProfitOfOneTransaction(vector<int> &prices, 
                vector<int> &bottom, vector<int> &top);
};

int Solution::maxProfit(vector<int> &prices) {
    vector<int> bottom, top;
    int oneProfit = maxProfitOfOneTransaction(prices, bottom, top);
    if (oneProfit == 0) return 0;
    if (bottom.size() < 2)  return oneProfit;
    int twoProfit = 0, tmpProfit = 0;
    map<int, int> profitTwoMap;
    for ( int i = 0; i < (signed)bottom.size()-1; i ++) {
        for (int j = i; j < (signed)top.size()-1; j ++) {
            tmpProfit = top[j] - bottom[i];
            if (profitTwoMap.find(j) == profitTwoMap.end()) {
                int tmpProfitTwo = 0;
                for (int k = j + 1; k < (signed)bottom.size(); k ++) {
                    for (int l = k; l < (signed)top.size(); l ++) {
                        if ((top[l] - bottom[k]) > tmpProfitTwo)
                            tmpProfitTwo = top[l] - bottom[k];
                    }
                }
                profitTwoMap[j] = tmpProfitTwo;
            }
            tmpProfit += profitTwoMap[j];
            if (tmpProfit > twoProfit)
                twoProfit = tmpProfit;
        }
    }
    if (oneProfit > twoProfit)
        return oneProfit;
    else 
        return twoProfit;
}
...
int Solution::maxProfitOfOneTransaction(vector<int> &prices, 
        vector<int> &bottom, vector<int> &top) {
    ...
    int i = 1;
    while (i < (signed)prices.size()) {
        int current = prices[i]; 
        if (current > formerPrice) {
            bottom.push_back(formerPrice);
            if ((formerPrice < min) || (min == -1) ) {
                if (max > min) {
                    int tmpProfit = max - min;
                    if (tmpProfit > profit) {
                        profit = tmpProfit;
                    }
                }
            ...
            formerPrice = current;
            if ( i != (signed)(prices.size() - 1)) {
                while ( (++i) < (signed)(prices.size() - 1)) {
                    if (prices[i] >= formerPrice) {
                        formerPrice = prices[i];
                        continue;
                    } else {
                        break;
                    }
                }
                if (prices[i] < formerPrice)    continue;
            }
            if (i == (signed)(prices.size() - 1)) {
                top.push_back(prices[i]);
                if (prices[i] > max)
                    max = prices[i];
            }
        } else if (current < formerPrice){
            if (min != -1)
                top.push_back(formerPrice);
            if ((formerPrice > max) && (min != -1)) { 
                max = formerPrice;
            }
            formerPrice = current;
            while ((i++) < (signed)(prices.size() - 1)) {
...
int main(int argc, char *argv[]) {
    Solution s;
    //int arr[] = {998, 78, 23, 50, 80, 34, 34, 78, 88, 99, 100, 101};
    //int arr[] = {1 , 2};
    //int arr[] = { 2, 1, 2, 0, 1};
