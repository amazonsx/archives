...
class Solution {
    public:
        ...
        vector<vector<string> > findLadders_TLE( string start, string end, unordered_set<string> &dict);
        vector<vector<string> > findLadders_MLE( string start, string end, unordered_set<string> &dict);
        vector<vector<string> > findLadders_LTE_and_BUG( string start, string end, unordered_set<string> &dict);
};
...
vector<vector<string> > Solution::findLadders( string start, string end, unordered_set<string> &dict){
    ...
    dict.erase(start);
    vector<string> data;
    map<string, unordered_set<string> > parent_m;
    vector<string> currentLevel, nextLevel;
    currentLevel.push_back(start);
    int finalLevelFlag = 0;
    ...
    while (currentLevel.size() != 0) {
        ...
        for ( unsigned int i = 0; i < currentLevel.size(); i ++)  {
            ...
            for (unsigned int j = 0; j < current.size(); j ++) {
                for ( char replace = 'a'; replace <= 'z'; replace ++) {
                    ...
                    if (replace == current[j])  continue;
                    string kid = current;
                    kid[j] = replace;
                    if (dict.find(kid) == dict.end())   continue;
                    if ( (kid == end) && (current == start))    continue;
                    if (parent_m.find(kid) == parent_m.end()) {
                        parent_m[kid] = tmp;
                    }

                    struct timeval timemiddle;
                    gettimeofday(&timemiddle, NULL);
                    cout << "The first  time is **********************    " 
                        << 1000000*(timemiddle.tv_sec - timebegin.tv_sec) + (timemiddle.tv_usec - timebegin.tv_usec) << endl;

                    parent_m[kid].insert(current); 
                    nextLevel.push_back(kid);
                    if (kid == end) finalLevelFlag = 1;

                    struct timeval timeend;
                    gettimeofday(&timeend, NULL);
                    cout << "The first  time is *****************************    " 
                        << 1000000*(timeend.tv_sec - timemiddle.tv_sec) + (timeend.tv_usec - timemiddle.tv_usec) << endl;
                }
            }
            dict.erase(current);
        }
        ...
        cout << "The first  time is ----------------    " 
            << 1000000*(second_time.tv_sec - first_time.tv_sec) + (second_time.tv_usec - first_time.tv_usec) << endl;

        cout << "nextLevel    size -    "  << nextLevel.size() << endl;
        currentLevel = nextLevel;
        struct timeval fourth;
        gettimeofday(&fourth, NULL);
        cout << "The fourth build time is --------------   " 
            << 1000000*(fourth.tv_sec - first_time.tv_sec) + (fourth.tv_usec - first_time.tv_usec) << endl;
        nextLevel.clear();
        
        struct timeval third_time;
        gettimeofday(&third_time, NULL);
        cout << "The second build time is --------------   " 
            << 1000000*(third_time.tv_sec - first_time.tv_sec) + (third_time.tv_usec - first_time.tv_usec) << endl;
    }
     
    struct timeval end_time;
    gettimeofday(&end_time, NULL);
    cout << "The parent_m build time is    " 
        << 1000000*(end_time.tv_sec - start_time.tv_sec) + (end_time.tv_usec - start_time.tv_usec) << endl;

    vector<vector<string> > res;
    for ( unordered_set<string>::iterator iter = parent_m[end].begin();
            iter != parent_m[end].end(); iter++) {
        vector<string> tmp;
        tmp.push_back(end);
        string current = *iter;
        tmp.insert( tmp.begin(), current);
        string parent = *(parent_m[current].begin());
        if (parent_m[current].size() > 1) 
            parent_m[current].erase(parent);
        while(parent != start) {
            current = parent;
            tmp.insert( tmp.begin(), current);
            parent = *(parent_m[current].begin());
            if (parent_m[current].size() > 1) 
                parent_m[current].erase(parent);
        }
    ...
    cout << "The parent_m build time is    " 
        << 1000000*(fin_time.tv_sec - start_time.tv_sec) + (fin_time.tv_usec - start_time.tv_usec) << endl;
...
struct WordNodeMLE {
    int index;
    int parent;
    WordNodeMLE(): index(0), parent(-1){}
    WordNodeMLE(int index, int parent): index(index), parent(parent) {}
};

struct WordNode {
    int parent;
    string current;
    WordNode(): parent(0), current(""){}
    WordNode(int parent, string current): parent(parent), current(current) {}
};

struct WordNodeNew {
    int level;
    string str;
    WordNodeNew(): level(0), str("") {}
    WordNodeNew(int level, string current):level(level), str(current) {}
};
vector<vector<string> > Solution::findLadders_TLE( string start, string end, unordered_set<string> &dict) {
    dict.insert(end); 
    vector<string> head;
    ...
    vector<string> currentLevel, nextLevel;
    currentLevel.push_back(start);
    map<string, vector<string> > current_connected_map;

    int final_level = 0, word_len = start.size(), level_count = 0;
    map<string, vector<WordNodeNew> > graph;
    WordNodeNew word_parent;
    struct timeval start_time;
    gettimeofday(&start_time, NULL);

    while( currentLevel.size() != 0) {
        if (final_level) break;
        unordered_set<string> nextLevelSet;
        for ( unsigned int i = 0; i < currentLevel.size(); i ++) { 
            string current = currentLevel[i];
            //cout << "The current str is--------" << current << endl;
            //struct timeval first_time;
            //gettimeofday(&first_time, NULL);
            if ( current_connected_map.find(current) != current_connected_map.end()) {
                for (unsigned int j = 0; j < current_connected_map[current].size(); j++) {
                    word_parent.level = level_count;
                    word_parent.str = current;
                    string kid = current_connected_map[current][j];
                    graph[kid].push_back(word_parent);
                    if (nextLevelSet.find(kid) == nextLevelSet.end()) {
                        nextLevelSet.insert(kid);
                        nextLevel.push_back(kid);
                    }
                    if (kid == end) final_level = 1;
                }
            } else {
                vector<string> tmp;
                current_connected_map[current] = tmp;
                for ( int j = 0; j < word_len; j ++) {
                    for ( char replace = 'a'; replace <= 'z'; replace ++) {
                        if (replace == current[j]) continue;
                        string new_word = current;
                        new_word[j] = replace;
                        if (dict.find(new_word) != dict.end()) {
                            if (graph.find(new_word) == graph.end()) {
                                vector<WordNodeNew> parents; 
                                graph[new_word] = parents;
                            }
                            word_parent.level = level_count;
                            word_parent.str = current;
                            graph[new_word].push_back(word_parent);
                            current_connected_map[current].push_back(new_word);
                            if (nextLevelSet.find(new_word) == nextLevelSet.end()) {
                                nextLevelSet.insert(new_word);
                                nextLevel.push_back(new_word);
                            }
                            if (new_word == end) final_level = 1;
                        }
                    }
                }
            }
            ...
            //gettimeofday( &second_time, NULL);
            //cout << "The time is ---------- " 
            //    << (second_time.tv_sec - first_time.tv_sec)*1000000 + (second_time.tv_usec-first_time.tv_usec) << endl;
        }
        currentLevel.clear();
        currentLevel = nextLevel;
        nextLevel.clear();
    ...
    gettimeofday(&end_time, NULL);
    cout << "The time is ---------- " 
        << (end_time.tv_sec - start_time.tv_sec)*1000000 + (end_time.tv_usec-start_time.tv_usec) << endl;
    int current_level;
    vector<vector<string> > res;
    for ( unsigned int i = 0; i < graph[end].size(); i ++) {
        string parent = graph[end][i].str;
        current_level = level_count;
        vector<string> path_v;
        path_v.push_back(end);
        while ( parent != start ) {
            path_v.insert(path_v.begin(), parent);
            current_level --;
            unsigned int i = 0;
            for ( ; i < graph[parent].size(); i ++) {
                if(graph[parent][i].level == (current_level-1)) { break; }
            }
            string former = parent;
            parent = graph[parent][i].str;
            if ( (i < graph[former].size()) && (graph[former][i+1].level == (current_level - 1))) 
                graph[former].erase(graph[former].begin() + i);
        }
        path_v.insert(path_v.begin(), start);
        res.push_back(path_v);
    }

    return res;
}

vector<vector<string> >  Solution::findLadders_LTE_and_BUG( string start, string end, unordered_set<string> &dict) {
    vector<vector<string> > res;
    vector<string> word_q;
    word_q.push_back(start);
    dict.insert(end);
    vector<WordNode> graph;
    WordNode head( -1, start);
    graph.push_back(head);
    int word_len = start.size(), final_round = 0;
    int current_size = 1, graph_cursor = 0;
    int tmp_size = 0;
    WordNode kid;
    string current, new_word;
    while( current_size != 0) {
        ...
        for ( int j = 0; j < current_size; j ++ ) {
            for ( int i = 0; i < word_len; i ++) {
                char replace = 'a';
                for ( ; replace <= 'z'; replace = replace + 1) {
                    if (replace == current[i]) continue;
                    new_word = current;
                    new_word[i] = replace;
                    if ( dict.find( new_word) != dict.end()) {
                        word_q.push_back(new_word);
                        kid.parent = graph_cursor;
                        kid.current = new_word;
                        graph.push_back(kid);
                        tmp_size ++;
                    }
                    if ( new_word == end)   final_round = 1;
                }
            }
            graph_cursor ++;
            if ( (j + 1) < (int)word_q.size()) current = word_q[j+1];
            word_q.erase(word_q.begin());
        }
        current_size = tmp_size;
    }
    for ( unsigned int i = 0; i < graph.size(); i ++ ) {
        if (graph[i].current != end) continue;
        vector<string> path;
        path.push_back(end);
        int parent = graph[i].parent;
        WordNode word = graph[parent];
        while (true) {
            path.insert(path.begin(), word.current);
            parent = word.parent;
            if (parent == -1) break;
            word = graph[parent];
        }
...
vector<vector<string> > Solution::findLadders_MLE( string start, string end, unordered_set<string> &dict) {
    vector<vector<string> > res;
    ...
    for ( unordered_set<string>::iterator iter = dict.begin(); 
        ...
        if ( *iter == start) start_index = count;
        if ( *iter == end) end_index = count;
        count ++;
    }
    ...
    for ( unsigned int i = 0; i < dict_v.size(); i ++) {
        a = dict_v[i];
        vector<int> connected_v;
        graph.push_back(connected_v);
        for ( unsigned int j = 0; j < dict_v.size(); j ++ ) {
            b = dict_v[j];
            int diff_count = 0;
            for ( int k = 0; k < word_len; k ++) {
                if (a[k] != b[k]) diff_count ++;
    ...
    vector<WordNodeMLE> word_q;
    WordNodeMLE head(start_index, -1);
    WordNodeMLE interval( -1, -1);
    word_q.push_back(head);
    word_q.push_back(interval);
    int current_i = 0, final_round = 0;
    WordNodeMLE current;
    while (current_i != (int)word_q.size()) {
        current = word_q[current_i];
        while (current.index != -1) {
            int parent = current.index;
            for( unsigned int i = 0; i < graph[parent].size(); i ++ ) {
                if (graph[parent][i] == end_index){
                    final_round = 1;
                }
                WordNodeMLE kid(graph[parent][i], current_i);
                word_q.push_back(kid);
            }
            current_i ++;
            current = word_q[current_i];
        }
    ...
    for ( int i = word_q.size() - 2; i >= 0; i -- ) {
        if (word_q[i].index == -1)  break;
        if (word_q[i].index != end_index) continue;
        vector<string> path;
        path.insert(path.begin(), dict_v[word_q[i].index]);
        int index = word_q[i].parent;
        while ( index != -1) {
            path.insert( path.begin(), dict_v[word_q[index].index]);
            index = word_q[index].parent;
        }
        res.push_back(path);
    }
