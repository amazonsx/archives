...
struct Edge {
	string u;
	string v;
	Edge(string u, string v):u(u), v(v){}
};

class Solution{
	private:
		int dijkstra( const vector<vector<int> > g, const int start, const int end);
		int bellman_ford( vector<Edge> edges, const string start, const string end, map<string, int> &res);
	public:
		int ladderLength(string start, string end, unordered_set<string> &dict);
		int ladderLength_bf(string start, string end, unordered_set<string> &dict);
...
int Solution::bellman_ford( vector<Edge> edges, const string start, const string end, map<string, int> &res){
	...
	map<string, int>::iterator res_iter = res.begin();
	vector<Edge>::iterator e_iter = edges.begin();
	int e_len = 1;
	int m_count = 0; // modify_count
	for (int i = 0; i < n; i ++) {
		m_count = 0;
		for ( e_iter = edges.begin(); e_iter != edges.end(); e_iter ++) {
			...
			v = e_iter->v;
			if ((res[u] != -1) 
					&& ((res[v] > (res[u] + e_len) ) || (res[v] == -1))) {
				res[v] = res[u] + e_len;
				m_count ++;
			} else if ((res[v] != -1) 
					&& ((res[u] > (res[v] + e_len) ) || (res[u] == -1))) {
				res[u] = res[v] + e_len;
				m_count ++;
			}
		}
...
int Solution::dijkstra( const vector<vector<int> > g, const int start, const int end){
	int n = g.size();
	int res[n];
	// complete flag
	int c_flag[n];
	...
	// complete_count
	int c_count = 0;
	// the second shortest point index
	int second_s_index = -1;
	while (c_count != n) {
		if (s_index == start) {
			res[s_index] = 0;
		}
		// find the new shortest point
		int n_s_index = second_s_index;
		//relax
		for (unsigned int i = 0; i < g[s_index].size(); i ++) {
			int j = g[s_index][i];
			if (c_flag[j] == 1 ) continue;
			if ((res[j] == -1) || ((1 + res[s_index]) < res[j])) {
				res[j] = 1 + res[s_index];
			} 
			if ( n_s_index == -1) {
				n_s_index = j;
			...
			if ( (res[j] < res[n_s_index])) {
				second_s_index = n_s_index;
				n_s_index = j;
			}
		}
		if (second_s_index == s_index) {
			second_s_index = -1;
			n_s_index = -1;
			for ( int k = 0; k < n; k ++) {
				if ((c_flag[k] == 1) || (res[k] == -1)) continue;
				if ((n_s_index == -1) && (second_s_index == -1) ) {
					second_s_index = k;
				} else {
					if (res[k] < res[second_s_index]) {
						if (n_s_index != -1) second_s_index = n_s_index;
						n_s_index = k;
					}
				}
			}
			if ( n_s_index == -1) {
				n_s_index = second_s_index;
				second_s_index = -1;
			}
		}
		c_flag[s_index] = 1;
		int pre_s_index = s_index;
		s_index = n_s_index;
		c_count ++;
		if (pre_s_index == end) break;
	}
...
int Solution::ladderLength_bf(string start, string end, unordered_set<string> &dict) {
	int len = dict.size();
	if (len == 0) return 0;
	// what if start == end
	dict.insert(start);
	...
	unordered_set<string>::iterator iter1 = dict.begin(), iter2 = dict.begin();
	int word_len = start.size();
	for ( int i = 0; i < len; i ++) {
		iter2 = iter1;
		for ( int j = i; j < len; j ++) {
	...
	return bellman_ford(edges, start, end, res) + 1;
}
...
int main(){
    ...
    */
	...
	string start("lost");
	string end("miss");
	string s[] = {"most","mist","miss","lost","fist","fish"};
    */
    /*
	string start("hot");
	string end("dog");
    ...
    */
    ...
    gettimeofday(&first, NULL);
	cout << so.ladderLength_bfs(start, end, dict) << endl;
    gettimeofday(&second, NULL);
    cout << "Time cost is ---- "  
        << 1000000*(second.tv_sec - first.tv_sec) + (second.tv_usec - first.tv_usec) << endl;

    gettimeofday(&first, NULL);
	cout << so.ladderLength_bf(start, end, dict) << endl;
    gettimeofday(&second, NULL);
    cout << "Time cost is ---- "  
        << 1000000*(second.tv_sec - first.tv_sec) + (second.tv_usec - first.tv_usec) << endl;
	return 1;
